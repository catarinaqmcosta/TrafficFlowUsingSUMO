...
if os.path.isfile('Output/results_sumo_realizations_varspeed_'+start_rlz+'-'+end_rlz+'_'+rnd_seed+'.csv') is False:
  with open('Output/results_sumo_realizations_varspeed_'+start_rlz+'-'+end_rlz+'_'+rnd_seed+'.csv', 'w', newline='') as sumo_output_file:
...
for i in range(nr_realiz):
  ...
  for j in curr_collapse_ids:
    ...
    if (len(closed_road_id) > 0):
      ...
      if (closed_road_id_str not in already_closed):
        ...
        # write xml with vss info
        child1 = etree.Element('variableSpeedSign', id="vss_"+str(j), lanes=closed_road_id_str+"_0")
        root.append(child1)
        child2 = etree.Element('step', time="0", speed="1.5")
  ...
  for row in curr_slight_dmg_df.itertuples(index = False):
    ...
    if (len(neighbor_edges) > 0 and row[2] not in curr_collapse_ids):
      ...
      if (vss_road_id not in already_closed_slight_dam and vss_road_id not in already_closed):
        ...
        root.append(child11)
        child12 = etree.Element('step', time="0", speed="4.86")
  ...
  # run SUMO
  subprocess.call(['sumo', '-c', 'run.sumo_messina_serverrun.sumocfg', '-a', 'variable_speed_'+start_rlz+'-'+end_rlz+'.xml,Output/taz_messina_v2-buffer.xml,Output/trips_messina_noshorttrips_taz_37k.xml', '--tripinfo-output', 'Results_SUMO/tripinfo_'+start_rlz+'-'+end_rlz+'.xml', '-l', 'Results_SUMO/sumo_log_'+start_rlz+'-'+end_rlz+'.txt'])
  ...
  with open('Output/results_closed_edges_varspeed_'+start_rlz+'-'+end_rlz+'_'+rnd_seed+'.csv', 'a') as closed_edges_file:
    closed_edges_file.write("\n".join(already_closed)+"\n")
  
  # append output to csv
  with open('Output/results_sumo_realizations_varspeed_'+start_rlz+'-'+end_rlz+'_'+rnd_seed+'.csv', 'a', newline='') as sumo_output_file:
    wr = csv.writer(sumo_output_file)
